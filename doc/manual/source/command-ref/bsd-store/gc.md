...
`bsd-store --gc` - run garbage collection

# Synopsis

`bsd-store` `--gc` [`--print-roots` | `--print-live` | `--print-dead`] [`--max-freed` *bytes*]

# Description

Without additional flags, the operation `--gc` performs a garbage
collection on the Bsd store. That is, all paths in the Bsd store not
...
`keep-outputs` and `keep-derivations` settings in the Bsd
configuration file.
...
```console
$ bsd-store --gc --max-freed $((100 * 1024 * 1024))
```
