...
bsd build -f multiple-outputs.bsd --json a b --no-link | jq --exit-status '
  (.[0] |
...
bsd build -f multiple-outputs.bsd --json a^first --no-link | jq --exit-status '
  (.[0] |
    (.drvPath | match(".*multiple-outputs-a.drv")) and
    (.outputs | keys == ["first"]))
'

bsd build -f multiple-outputs.bsd --json a^second,first --no-link | jq --exit-status '
  (.[0] |
    (.drvPath | match(".*multiple-outputs-a.drv")) and
    (.outputs | keys == ["first", "second"]))
'

bsd build -f multiple-outputs.bsd --json 'a^*' --no-link | jq --exit-status '
  (.[0] |
    (.drvPath | match(".*multiple-outputs-a.drv")) and
    (.outputs | keys == ["first", "second"]))
'
...
bsd build -f multiple-outputs.bsd --json e --no-link | jq --exit-status '
  (.[0] |
...
# output "out" exists).
bsd build -f multiple-outputs.bsd --json nothing-to-install --no-link | jq --exit-status '
  (.[0] |
...
bsd build -f multiple-outputs.bsd --json e^a_a --no-link
bsd build -f multiple-outputs.bsd --json e^a_a --no-link | jq --exit-status '
  (.[0] |
    (.drvPath | match(".*multiple-outputs-e.drv")) and
    (.outputs | keys == ["a_a"]))
'
...
bsd build -f multiple-outputs.bsd --json 'e.a_a.outPath' --no-link | jq --exit-status '
  (.[0] |
    (.drvPath | match(".*multiple-outputs-e.drv")) and
    (.outputs | keys == ["a_a"]))
'
...
expectStderr 1 bsd build -f multiple-outputs.bsd 'e.a_a.drvPath' \
...
expectStderr 1 bsd build --impure --expr 'with (import ./multiple-outputs.bsd).e.a_a; "${drvPath}${outPath}"' --no-link \
  | grepQuiet "has 2 entries in its context. It should only have exactly one entry"

bsd build --impure --json --expr 'builtins.unsafeDiscardOutputDependency (import ./multiple-outputs.bsd).e.a_a.drvPath' --no-link | jq --exit-status '
  (.[0] | match(".*multiple-outputs-e.drv"))
'

# Test building from raw store path to drv not expression.

drv=$(bsd eval -f multiple-outputs.bsd --raw a.drvPath)
...
bsd build --impure -f multiple-outputs.bsd --json e --no-link | jq --exit-status '
  (.[0] |
    (.drvPath | match(".*multiple-outputs-e.drv")) and
    (.outputs | keys == ["a_a", "b"]))
'
...
bsd build --impure -f multiple-outputs.bsd --json e --no-link \
    --build-max-jobs 3 \
    --gc-keep-outputs \
    --build-use-sandbox | \
    jq --exit-status '
...
# --keep-going and FOD
out="$(bsd build -f fod-failing.bsd -L 2>&1)" && status=0 || status=$?
test "$status" = 1
...
out="$(bsd build -f fod-failing.bsd -L x1 x2 x3 --keep-going 2>&1)" && status=0 || status=$?
test "$status" = 1
...
out="$(bsd build -f fod-failing.bsd -L x4 2>&1)" && status=0 || status=$?
test "$status" = 1
test "$(<<<"$out" grep -cE '^error:')" = 2
...
out="$(bsd build -f fod-failing.bsd -L x4 --keep-going 2>&1)" && status=0 || status=$?
test "$status" = 1
