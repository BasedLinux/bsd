...
outPath=$(bsd-build ./export-graph.nix -A 'foo."bar.runtimeGraph"' -o $TEST_ROOT/result)
...
outPath=$(bsd-build ./export-graph.nix -A 'foo."bar.buildGraph"' -o $TEST_ROOT/result)
