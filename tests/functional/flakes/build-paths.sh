...
bsd build --json --out-link "$TEST_ROOT/result" "$flake1Dir#a3"
...
expectStderr 1 bsd build --impure --json --out-link "$TEST_ROOT/result" "$flake1Dir#a9" \
  | grepQuiet "has 0 entries in its context. It should only have exactly one entry"

bsd build --json --out-link "$TEST_ROOT/result" "$flake1Dir"#a10
[[ $(readlink -e "$TEST_ROOT/result") = *simple.drv ]]

expectStderr 1 bsd build --json --out-link "$TEST_ROOT/result" "$flake1Dir#a11" \
  | grepQuiet "has a context which refers to a complete source and binary closure"

bsd build --json --out-link "$TEST_ROOT/result" "$flake1Dir#a12"
[[ -e $TEST_ROOT/result/hello ]]

expectStderr 1 bsd build --impure --json --out-link "$TEST_ROOT/result" "$flake1Dir#a13" \
...
bsd build --json --no-link "$flake1Dir"#a14.foo | jq --exit-status '
  (.[0] |
