...
expectStderr 0 bsd --offline build --dry-run "git+file://$repoDir#foo"

# Check that a flakeref with a good query is accepted correctly.
expectStderr 0 bsd --offline build --dry-run "git+file://$repoDir?foo=bar#foo"

# Check that we get the dubious query warning, when passing in a query without an equal sign.
expectStderr 0 bsd --offline build --dry-run "git+file://$repoDir?bar#foo" \
...
expectStderr 1 bsd --offline build --dry-run "git+file://$repoDir#foo?bar" \
