...
EOF
bsd run --no-write-lock-file .#appAsApp
bsd run --no-write-lock-file .#pkgAsPkg

! bsd run --no-write-lock-file .#pkgAsApp || fail "'bsd run' shouldn’t accept an 'app' defined under 'packages'"
! bsd run --no-write-lock-file .#appAsPkg || fail "elements of 'apps' should be of type 'app'"
...
# bsd run appears to clear _. I don't understand why. Is this ok?
