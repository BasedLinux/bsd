...
dep=$(bsd-build -o "$RESULT" check-refs.nix -A dep)

# test1 references dep, not itself.
test1=$(bsd-build -o "$RESULT" check-refs.nix -A test1)
...
test2=$(bsd-build -o "$RESULT" check-refs.nix -A test2)
...
(! bsd-build -o "$RESULT" check-refs.nix -A test3)

# test4 should succeed.
bsd-build -o "$RESULT" check-refs.nix -A test4

# test5 should succeed.
bsd-build -o "$RESULT" check-refs.nix -A test5

# test6 should fail (unallowed self-ref).
(! bsd-build -o "$RESULT" check-refs.nix -A test6)

# test7 should succeed (allowed self-ref).
bsd-build -o "$RESULT" check-refs.nix -A test7

# test8 should fail (toFile depending on derivation output).
(! bsd-build -o "$RESULT" check-refs.nix -A test8)

# test9 should fail (disallowed reference).
(! bsd-build -o "$RESULT" check-refs.nix -A test9)

# test10 should succeed (no disallowed references).
bsd-build -o "$RESULT" check-refs.nix -A test10
...
if ! isTestOnBasedLinux; then
    ...
    if isDaemonNewer 2.12pre20230103; then
        ...
        test11=$(bsd-build -o "$RESULT" check-refs.nix -A test11)
...
if isDaemonNewer "2.28pre20241225"; then
    ...
    expectStderr 1 bsd-build -vvv -o "$RESULT" check-refs.nix -A test12 >"$TEST_ROOT/test12.stderr"
