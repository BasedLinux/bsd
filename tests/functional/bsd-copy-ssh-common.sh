...
bsd copy "${args[@]}" --to "$remoteStore" $outPath
[ -f ${remoteRoot}${outPath}/foobar ]
...
[ ! -f $outPath/foobar ]
bsd copy --no-check-sigs --from "$remoteStore" $outPath
[ -f $outPath/foobar ]
...
bsd copy --no-check-sigs "$outPath" --to "$corruptedStore"
...
NIX_CONFIG=$(echo -e "substituters = local\nrequire-sigs = false") \
    bsd copy --no-check-sigs --from "$corruptedStore" --to "$remoteStore" --substitute-on-destination "$outPath"
