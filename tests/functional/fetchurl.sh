...
hash=$(bsd hash file ./fetchurl.sh)
...
hash=$(bsd hash file --type sha256 --base16 ./fetchurl.sh)
...
bsd-store --dump "$TEST_ROOT/archive" > "$nar"
...
expectStderr $expected bsd-build --expr '{ url }: builtins.derivation { name = "bsd-cache-info"; system = "x86_64-linux"; builder = "builtin:fetchurl"; inherit url; outputHashMode = "flat"; }' --argstr url "file://$narxz" 2>&1 | grep 'must be a fixed-output or impure derivation'
