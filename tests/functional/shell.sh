...
bsd env shell -f shell-hello.nix hello -c hello | grep 'Hello World'

bsd shell -f shell-hello.nix hello -c hello | grep 'Hello World'
bsd shell -f shell-hello.nix hello -c hello BasedLinux | grep 'Hello BasedLinux'

# Test output selection.
bsd shell -f shell-hello.nix hello^dev -c hello2 | grep 'Hello2'
bsd shell -f shell-hello.nix 'hello^*' -c hello2 | grep 'Hello2'

# Test output paths that are a symlink.
bsd shell -f shell-hello.nix hello-symlink -c hello | grep 'Hello World'

# Test that symlinks outside of the store don't work.
expect 1 bsd shell -f shell-hello.nix forbidden-symlink -c hello 2>&1 | grepQuiet "is not in the Bsd store"
...
bsd shell -f shell-hello.nix hello -c env > "$TEST_ROOT/actual-env"
...
if isDaemonNewer "2.20.0pre20231220"; then
    ...
    bsd shell -f shell-hello.nix hello salve-mundi -c hello | grep 'Hello World'
    bsd shell -f shell-hello.nix salve-mundi hello -c hello | grep 'Salve Mundi'
fi
...
path=$(bsd eval --raw -f shell-hello.nix hello)
...
path2=$(bsd shell --sandbox-paths '/bsd? /bin? /lib? /lib64? /usr?' --store "$TEST_ROOT/store0" -f shell-hello.nix hello -c "$SHELL" -c 'type -p hello')
