...
test_fetch_file () {
    ...
    input_hash="$(bsd hash path test_input)"

    bsd eval --impure --file - <<EOF
    let
...
test_file_flake_input () {
    ...
    echo '{ outputs = { self }: { }; }' > inputs/flake.bsd
    tar cfa test_input.tar.gz inputs
    cp test_input.tar.gz test_input_no_ext
    input_tarball_hash="$(bsd hash path test_input.tar.gz)"
    input_directory_hash="$(bsd hash path inputs)"
...
EOF
    ...
    # because that would break older Bsd versions
...
EOF
    ...
    [[ $(bsd flake metadata --json "file://$PWD/test_input_no_ext" | jq -r .resolved.type) = tarball ]]

    popd

    [[ -z "${NIX_DAEMON_PACKAGE-}" ]] && return 0

    # Ensure that a lockfile generated by the current Bsd for tarball inputs
    # can still be read by an older Bsd
