...
#     bsd --experimental-features 'bsd-command' "$@" | grepQuietInverse flake
#     bsd --experimental-features 'bsd-command flakes' "$@" | grepQuiet flake
#
...
' expect 1 bsd config show accept-flake-config 1>"$TEST_ROOT"/stdout 2>"$TEST_ROOT"/stderr
...
NIX_CONFIG='
  ...
  accept-flake-config = true
' bsd config show accept-flake-config 1>"$TEST_ROOT"/stdout 2>"$TEST_ROOT"/stderr
...
function exit_code_both_ways {
    expect 1 bsd --experimental-features 'bsd-command' "$@" 1>/dev/null
    bsd --experimental-features 'bsd-command flakes' "$@" 1>/dev/null
...
bsd --experimental-features '' --help 1>/dev/null
bsd --experimental-features '' doctor --help 1>/dev/null
bsd --experimental-features '' repl --help 1>/dev/null
bsd --experimental-features '' upgrade-bsd --help 1>/dev/null
...
# bsd-command experimental features --- it so happens that the commands we wish
# stabilizing to do not need them anyways.
for arg in '--print-build-logs' '--offline' '--refresh'; do
    bsd --experimental-features 'bsd-command' "$arg" --help 1>/dev/null
    expect 1 bsd --experimental-features '' "$arg" --help 1>/dev/null
done
