...
happy () {
    ...
    bsd --store local?read-only=true path-info $dummyPath

    # `local://` also works.
    bsd --store local://?read-only=true path-info $dummyPath

    # We can "write" an already-present store-path a read-only store, because no IO is actually required
    bsd-store --store local?read-only=true --add dummy
}
...
expectStderr 1 bsd-store --store local?read-only=true --add dummy | grepQuiet "database does not exist, and cannot be created in read-only mode"
