...
outPath="$(NIX_REMOTE='local?store=/foo&real='"$TEST_ROOT"'/real-store' bsd-instantiate --readonly-mode hash-check.nix)"
if test "$outPath" != "/foo/lfy1s6ca46rm5r6w4gg9hc0axiakjcnm-dependencies.drv"; then
    echo "hashDerivationModulo appears broken, got $outPath"
    exit 1
fi

outPath="$(NIX_REMOTE='local?store=/foo&real='"$TEST_ROOT"'/real-store' bsd-instantiate --readonly-mode big-derivation-attr.nix)"
