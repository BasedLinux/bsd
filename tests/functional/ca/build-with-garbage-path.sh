...
outPath="$(bsd-build ./content-addressed.nix -A rootCA --no-out-link)"
...
bsd-build ./content-addressed.nix -A rootCA --no-out-link

# Rebuild it. This shouldn’t overwrite the existing path
oldInode=$(stat -c '%i' "$outPath")
bsd-build ./content-addressed.nix -A rootCA --no-out-link --arg seed 2
