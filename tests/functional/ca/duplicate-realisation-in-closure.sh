...
bsd copy --to "$REMOTE_STORE" -f nondeterministic.nix dep1
clearStore
...
# realisation
bsd build -f nondeterministic.nix dep2 --no-link
...
# and have `dep1 == dep2`.
bsd build --substituters "$REMOTE_STORE" -f nondeterministic.nix toplevel --no-require-sigs --no-link
