...
bsd copy --to "$REMOTE_STORE" -f nondeterministic.bsd dep1
clearStore
...
# realisation
bsd build -f nondeterministic.bsd dep2 --no-link
...
# and have `dep1 == dep2`.
bsd build --substituters "$REMOTE_STORE" -f nondeterministic.bsd toplevel --no-require-sigs --no-link
