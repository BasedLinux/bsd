...
buildDrvs () {
    bsd build --file ./content-addressed.nix -L --no-link "$@"
}

# Populate the remote cache
clearStore
bsd copy --to "$REMOTE_STORE" --file ./content-addressed.nix
...
bsd realisation info --file ./content-addressed.nix transitivelyDependentCA
# Check that its dependencies have it too
bsd realisation info --file ./content-addressed.nix dependentCA
# bsd realisation info --file ./content-addressed.nix rootCA --outputs out
...
clearStore
bsd build --file ../simple.nix -L --no-link --post-build-hook "$pushToStore"
clearStore
rm -r "$REMOTE_STORE_DIR/realisations"
bsd build --file ../simple.nix -L --no-link --substitute --substituters "$REMOTE_STORE" --no-require-sigs -j0
...
bsd copy --to "$REMOTE_STORE" --file ../simple.nix
