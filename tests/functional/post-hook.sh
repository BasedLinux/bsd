...
bsd-build -o $TEST_ROOT/result dependencies.bsd --post-build-hook "$pushToStore"
...
bsd-build -o $TEST_ROOT/result-mult multiple-outputs.bsd -A a.first --post-build-hook "$pushToStore"

clearStore

# Ensure that the remote store contains both the runtime and build-time
# closure of what we've just built.
bsd copy --from "$REMOTE_STORE" --no-require-sigs -f dependencies.bsd
bsd copy --from "$REMOTE_STORE" --no-require-sigs -f dependencies.bsd input1_drv
bsd copy --from "$REMOTE_STORE" --no-require-sigs -f multiple-outputs.bsd a^second
