...
echo 'require-sigs = false' >> $test_bsd_conf
...
bsd-build -o $TEST_ROOT/result dependencies.nix --post-build-hook "$pushToStore"
...
bsd-build -o $TEST_ROOT/result-mult multiple-outputs.nix -A a.first --post-build-hook "$pushToStore"

clearStore

# Ensure that the remote store contains both the runtime and build-time
# closure of what we've just built.
bsd copy --from "$REMOTE_STORE" --no-require-sigs -f dependencies.nix
bsd copy --from "$REMOTE_STORE" --no-require-sigs -f dependencies.nix input1_drv
bsd copy --from "$REMOTE_STORE" --no-require-sigs -f multiple-outputs.nix a^second
