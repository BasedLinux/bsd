...
expectStderr 1 bsd-store --delete $input2 | grepQuiet "Cannot delete path"
expectStderr 1 bsd-store --delete $input3 | grepQuiet "Cannot delete path"

# These same paths are referenced in the lower layer (by the seed 1
# build done in `initLowerStore`).
expectStderr 1 bsd-store --store "$storeA" --delete $input2 | grepQuiet "Cannot delete path"
expectStderr 1 bsd-store --store "$storeA" --delete $input3 | grepQuiet "Cannot delete path"

# Can delete
bsd-store --delete $hermetic

# Now unreferenced in upper layer, can delete
bsd-store --delete $input3
bsd-store --delete $input2
