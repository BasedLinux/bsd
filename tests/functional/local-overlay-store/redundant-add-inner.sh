...
pathInLowerStore=$(bsd-store --store "$storeA" --add ../dummy)

# upper layer should not have it
expect 1 stat $(toRealPath "$storeBTop/bsd/store" "$pathInLowerStore")

pathFromB=$(bsd-store --store "$storeB" --add ../dummy)
