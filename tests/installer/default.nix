...
let
  ...
  installScripts = {
    ...
    install-both-profile-links = {
      script = ''
        tar -xf ./bsd.tar.xz
        mv ./bsd-* bsd
        ln -s $HOME/.local/state/bsd/profiles/a-profile $HOME/.bsd-profile
  ...
  mockChannel =
    ...
    pkgs.runCommandNoCC "mock-channel" { } ''
      mkdir bsdexprs
      mkdir -p $out/channel
      echo -n 'someContent' > bsdexprs/someFile
      tar cvf - bsdexprs | bzip2 > $out/channel/bsdexprs.tar.bz2
    '';
  ...
  makeTest =
    ...
    in
    with bsdpkgsFor.${image.system}.native;
    runCommand "installer-test-${imageName}-${testName}"
      ...
      ''
        ...
        scp -P 20022 $ssh_opts $binaryTarball/bsd-*.tar.xz vagrant@localhost:bsd.tar.xz
        ...
        $ssh <<EOF
          ...
          bsd-env --version
          bsd --extra-experimental-features bsd-command store info

          out=\$(bsd-build --no-substitute -E 'derivation { name = "foo"; system = "x86_64-linux"; builder = "/bin/sh"; args = ["-c" "echo foobar > \$out"]; }')
          [[ \$(cat \$out) = foobar ]]

          if pgrep bsd-daemon; then
            MAYBESUDO="sudo"
          else
            MAYBESUDO=""
          fi
          ...
          $MAYBESUDO \$(which bsd-channel) --add file://\$HOME/channel myChannel
          $MAYBESUDO \$(which bsd-channel) --update
          [[ \$(bsd-instantiate --eval --expr 'builtins.readFile <myChannel/someFile>') = '"someContent"' ]]
        EOF
