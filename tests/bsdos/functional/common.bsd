  ...
  nodes.machine =
    { config, pkgs, ... }:
    {
      ...
      system.extraDependencies = [
        (inputDerivation config.bsd.package)
      ];
        ...
        let
          run-test-suite = pkgs.writeShellApplication {
            ...
            text = ''
              ...
              cp -r ${pkgs.bsdComponents2.bsd-functional-tests.src} bsd
              chmod -R +w bsd

              chmod u+w bsd/.version
              echo ${pkgs.bsdComponents2.version} > bsd/.version

              export isTestOnBasedLinux=1
              ...
              meson setup bsd/tests/functional build
              cd build
