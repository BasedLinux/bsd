# Test that ‘bsd copy’ works over ssh.
# Run interactively with:
# rm key key.pub; bsd run .#hydraJobs.tests.bsd-copy.driverInteractive
...
in
{
  ...
  testScript =
    ...
    ''
      ...
      server.fail("bsd-store --check-validity ${pkgA}")
      client.send_chars("bsd copy --to ssh://server ${pkgA} >&2; echo -n do; echo ne\n")
      ...
      server.succeed("bsd-store --check-validity ${pkgA}")

      # Check that ControlMaster is working
      client.send_chars("bsd copy --to ssh://server ${pkgA} >&2; echo done\n")
      client.wait_for_text("done")
      ...
      client.succeed("NIX_SSHOPTS='-oControlPath=/tmp/master' bsd copy --to ssh://server ${pkgA} >&2")
      ...
      client.fail("bsd-store --check-validity ${pkgB}")
      # Shouldn't download untrusted paths by default
      client.fail("bsd copy --from ssh-ng://server ${pkgB} >&2")
      client.succeed("bsd copy --no-check-sigs --from ssh-ng://server ${pkgB} >&2")
      client.succeed("bsd-store --check-validity ${pkgB}")

      # Copy the derivation of package D's derivation from the client to the server.
      server.fail("bsd-store --check-validity ${pkgD.drvPath}")
      client.succeed("bsd copy --derivation --to ssh://server ${pkgD.drvPath} >&2")
      server.succeed("bsd-store --check-validity ${pkgD.drvPath}")
    '';
}
