# Test Bsd's remote build feature.
...
let
  ...
  supportsBadShell = lib.versionAtLeast config.nodes.client.bsd.package.version "2.25pre";
  ...
  config = {
      ...
      client =
        ...
        }:
        {
          ...
          environment.systemPackages = [
            # `bad-shell` is used to make sure Bsd works in an environment with a misbehaving shell.
            #
    ...
    testScript =
      ...
      ''
        ...
        out = client.succeed(
          "${lib.optionalString supportsBadShell "SHELL=$(which bad-shell)"} bsd-build ${expr nodes.client 1} 2> build-output",
          "grep -q Hello build-output"
        )
        builder1.succeed(f"test -e {out}")

        # And a parallel build.
        paths = client.succeed(r'bsd-store -r $(bsd-instantiate ${expr nodes.client 2})\!out $(bsd-instantiate ${expr nodes.client 3})\!out')
        out1, out2 = paths.split()
        builder1.succeed(f"test -e {out1} -o -e {out2}")
        builder2.succeed(f"test -e {out1} -o -e {out2}")

        # And a failing build.
        client.fail("bsd-build ${expr nodes.client 5}")

        # Test whether the build hook automatically skips unavailable builders.
        builder1.block()
        client.succeed("bsd-build ${expr nodes.client 4}")
      '';
  };
}
