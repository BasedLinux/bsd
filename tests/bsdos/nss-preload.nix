...
let
  ...
  bsd-fetch = pkgs.writeText "fetch.bsd" ''
    derivation {
        # This derivation is an copy from what is available over at
        # bsd.git:corepkgs/fetchurl.bsd
        ...
        # ```
        # $ bsd-hash --type sha256 --to-base32 $(echo "hello world" | sha256sum | cut -d " " -f 1)
  ...
  nodes = {
          ...
          settings = {
            server = {
              ...
              local-data = [
                ...
                ''"tarballs.basedlinux.org. IN A 192.168.0.1"''
                ''"tarballs.basedlinux.org. IN AAAA fd21::1"''
              ];
            };
          };
        };
    ...
    client =
      ...
      }:
      {
        ...
        bsd.settings.extra-sandbox-paths = lib.mkForce [ ];
        bsd.settings.substituters = lib.mkForce [ ];
        bsd.settings.sandbox = lib.mkForce true;
      };
  };
  ...
  testScript =
    ...
    ''
      ...
      with subtest("can fetch data from a remote server outside sandbox"):
          client.succeed("bsd --version >&2")
          client.succeed("curl -vvv http://example.com/index.html >&2")

      with subtest("bsd-build can lookup dns and fetch data"):
          client.succeed("""
            bsd-build ${bsd-fetch} >&2
            """)
    '';
}
