...
let
  ...
  # Generate a fake root CA and a fake api.github.com / github.com / channels.basedlinux.org certificate.
  cert = pkgs.runCommand "cert" { nativeBuildInputs = [ pkgs.openssl ]; } ''
    ...
    openssl x509 -req -extfile <(printf "subjectAltName=DNS:api.github.com,DNS:github.com,DNS:channels.basedlinux.org") \
  ...
  private-flake-api = pkgs.runCommand "private-flake" { } ''
    ...
    dir=private-flake
    mkdir $dir
    echo '{ outputs = {...}: {}; }' > $dir/flake.bsd
    tar cfz $out/tarball/${private-flake-rev} $dir --hard-dereference
  '';

  bsdpkgs-api = pkgs.runCommand "bsdpkgs-flake" { } ''
    mkdir -p $out/commits

    # Setup https://docs.github.com/en/rest/commits/commits#get-a-commit
    echo '{"sha": "${bsdpkgs.rev}", "commit": {"tree": {"sha": "ffffffffffffffffffffffffffffffffffffffff"}}}' > $out/commits/HEAD
  '';

  archive = pkgs.runCommand "bsdpkgs-flake" { } ''
    mkdir -p $out/archive
...

{
  ...
  nodes = {
    github =
      { config, pkgs, ... }:
      {
        ...
        '';
        services.httpd.virtualHosts."channels.basedlinux.org" = {
          forceSSL = true;
        ...
        services.httpd.virtualHosts."api.github.com" = {
          ...
          servedDirs = [
            {
              urlPath = "/repos/BasedLinux/bsdpkgs";
              dir = bsdpkgs-api;
            }
            {
  ...
  testScript =
    ...
    ''
      ...
      out = client.succeed("bsd registry list")
      print(out)
      assert "github:BasedLinux/bsdpkgs" in out, "bsdpkgs flake not found"
      assert "github:fancy-enterprise/private-flake" in out, "private flake not found"
      cat_log()

      # If no github access token is provided, bsd should use the public archive url...
      out = client.succeed("bsd flake metadata bsdpkgs --json")
      print(out)
      info = json.loads(out)
      assert info["revision"] == "${bsdpkgs.rev}", f"revision mismatch: {info['revision']} != ${bsdpkgs.rev}"
      cat_log()

      # ... otherwise it should use the API
      out = client.succeed("bsd flake metadata private-flake --json --access-tokens github.com=ghp_000000000000000000000000000000000000 --tarball-ttl 0")
      print(out)
      info = json.loads(out)
      ...
      info2 = json.loads(client.succeed(f"bsd flake metadata {info['url']} --json --access-tokens github.com=ghp_000000000000000000000000000000000000 --tarball-ttl 0"))
      ...
      client.succeed("bsd flake metadata bsdpkgs --tarball-ttl 0 >&2")

      # Test fetchTree on a github URL.
      hash = client.succeed(f"bsd eval --no-trust-tarballs-from-git-forges --raw --expr '(fetchTree {info['url']}).narHash'")
      assert hash == info['locked']['narHash']

      # Fetching without a narHash should succeed if trust-github is set and fail otherwise.
      client.succeed(f"bsd eval --raw --expr 'builtins.fetchTree github:github:fancy-enterprise/private-flake/{info['revision']}'")
      out = client.fail(f"bsd eval --no-trust-tarballs-from-git-forges --raw --expr 'builtins.fetchTree github:github:fancy-enterprise/private-flake/{info['revision']}' 2>&1")
      ...
      info = json.loads(client.succeed("bsd flake metadata bsdpkgs --json"))
