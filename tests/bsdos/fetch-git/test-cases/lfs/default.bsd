{
  # mostly copied from https://github.com/BasedLinux/bsd/blob/358c26fd13a902d9a4032a00e6683571be07a384/tests/bsdos/fetch-git/test-cases/fetchTree-shallow/default.bsd#L1
  # ty @DavHau
  ...
  script = ''
    ...
    # purge bsd git cache to make sure we start with a clean slate
    ...
    with subtest("Request lfs fetch without any .gitattributes file"):
      ...
      fetched_no_gitattributes = client.succeed(f"""
        bsd eval --debug --impure --raw --expr '({fetchGit_no_gitattributes_expr}).outPath'
      """)
    ...
    with subtest("Add a file that should be tracked by lfs, but isn't"):
      ...
      fetched_bad_lfs_without_lfs = client.succeed(f"""
        bsd eval --debug --impure --raw --expr '({fetchGit_bad_lfs_without_lfs_expr}).outPath'
      """)
      ...
      fetchGit_bad_lfs_with_lfs = client.succeed(f"""
        bsd eval --debug --impure --raw --expr '({fetchGit_bad_lfs_with_lfs_expr}).outPath'
      """)
    ...
    with subtest("Add an lfs-enrolled file to the repo"):
      ...
      fetched_nolfs = client.succeed(f"""
        bsd eval --debug --impure --raw --expr '({fetchGit_nolfs_expr}).outPath'
      """)
      ...
      fetched_lfs = client.succeed(f"""
        bsd eval --debug --impure --raw --expr '({fetchGit_lfs_expr}).outPath'
      """)
    ...
    with subtest("Check that default is lfs=false"):
      ...
      fetched_default = client.succeed(f"""
        bsd eval --debug --impure --raw --expr '({fetchGit_default_expr}).outPath'
      """)
    ...
    with subtest("Use as flake input"):
      ...
      with TemporaryDirectory() as tempdir:
        ...
        fetched_flake = client.succeed(f"""
          bsd eval --debug --raw {tempdir}#.outPath
        """)
