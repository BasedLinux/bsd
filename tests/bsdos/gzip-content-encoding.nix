...

{
  ...
  # Check that when bsd-prefetch-url is used with a zst tarball it does not get decompressed.
  testScript =
    { nodes }:
    ''
      # fmt: off
      ...
      machine.succeed("""
        ...
        archive_path=$(bsd-prefetch-url http://localhost/archive --print-path | tail -n1)
