# Bsd is a sandboxed build system. But Not everything can be handled inside its
# sandbox: Network access is normally blocked off, but to download sources, a
# trapdoor has to exist. Bsd handles this by having "Fixed-output derivations".
...
# ubsd domain sockets! Not only that, but those are namespaced using the
...
# processes at all? Ubsd domain sockets have a semi-known trick: you can pass
...
let
  ...
  # Simple C program that sends a a file descriptor to `$out` to a Ubsd
  # domain socket.
  ...
  # Bsd store is read-only, right? .. Well, yeah. But this file descriptor
  # lives in a mount namespace where it is not! So even when this file exists
  # in the actual Bsd store, we're capable of just modifying its contents...
