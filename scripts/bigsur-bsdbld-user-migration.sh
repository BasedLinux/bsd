...
change_bsdbld_names_and_ids(){
	local name uid next_id
	((next_id=NEW_NIX_FIRST_BUILD_UID))
	echo "Attempting to migrate bsdbld users."
	echo "Each user should change from bsdbld# to _bsdbld#"
	...
	while read -r name uid; do
		...
		if [[ "$name" == _* ]]; then
			...
			sudo dseditgroup -o edit -t user -d "$name" bsdbld || true
			...
			sudo dseditgroup -o edit -t user -a "_$name" bsdbld
			echo "      $name migrated to _$name (uid: $next_id)"
		fi
	done < <(dscl . list /Users UniqueID | grep bsdbld | sort -n -k2)
}
