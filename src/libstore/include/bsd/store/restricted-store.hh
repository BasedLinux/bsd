...
struct RestrictionContext
{
    ...
    /**
     * Paths that were added via recursive Bsd calls.
     */
    StorePathSet addedPaths;

    /**
     * Realisations that were added via recursive Bsd calls.
     */
    std::set<DrvOutput> addedDrvOutputs;

    /**
     * Recursive Bsd calls are only allowed to build or realize paths
     * in the original input closure or added via a recursive Bsd call
     * (so e.g. you can't do 'bsd-store -r /bsd/store/<bla>' where
     * /bsd/store/<bla> is some arbitrary path in a binary cache).
     */
