...
public:
     ...
     */
    std::vector<Path> bsdUserConfFiles;

    /**
     * File name of the socket the daemon listens to.
     */
    Path bsdDaemonSocketFile;
    ...
    Setting<bool> tryFallback{
        this, false, "fallback",
        R"(
          If set to `true`, Bsd falls back to building from source if a
    ...
    MaxBuildJobsSetting maxBuildJobs{
        this, 1, "max-jobs",
        R"(
          Maximum number of jobs that Bsd tries to build locally in parallel.

          The special value `auto` causes Bsd to use the number of CPUs in your system.
    ...
    Setting<unsigned int> maxSubstitutionJobs{
        ...
        R"(
          This option defines the maximum number of substitution jobs that Bsd
    ...
    Setting<unsigned int> buildCores{
        ...
        R"(
          ...
          -->
          For instance, in Bsdpkgs, if the attribute `enableParallelBuilding` for the `mkDerivation` build helper is set to `true`, it passes the `-j${NIX_BUILD_CORES}` flag to GNU Make.

          The value `0` means that the `builder` should use all available CPU cores in the system.

          > **Note**
          >
          > The number of parallel local Bsd build jobs is independently controlled with the [`max-jobs`](#conf-max-jobs) setting.
        )",
        {"build-cores"},
    ...
    Setting<std::string> thisSystem{
        ...
        R"(
          The system type of the current Bsd installation.
          Bsd only builds a given [store derivation](@docroot@/glossary.md#gloss-store-derivation) locally when its `system` attribute equals any of the values specified here or in [`extra-platforms`](#conf-extra-platforms).

          The default value is set when Bsd itself is compiled for the system it will run on.
          The following system types are widely used, as Bsd is actively supported on these platforms:
          ...
          While you can force Bsd to run a Darwin-specific `builder` executable on a Linux machine, the result would obviously be wrong.

          This value is available in the Bsd language as
    ...
    Setting<Strings> buildHook{this, {"bsd", "__build-remote"}, "build-hook",
        R"(
          The path to the helper program that executes remote builds.

          Bsd communicates with the build hook over `stdio` using a custom protocol to request builds that cannot be performed directly by the Bsd daemon.
          The default value is the internal Bsd binary that implements remote building.
    ...
    Setting<std::string> builders{
        this, "@" + bsdConfDir + "/machines", "builders",
        R"(
          ...
          2. A comma-separated list of [Bsd system types](@docroot@/development/building.md#system-type).
          ...
          4. The maximum number of builds that Bsd executes in parallel on the machine.
          ...
          5. The “speed factor”, indicating the relative speed of the machine as a positive integer.
             If there are multiple machines of the right type, Bsd prefers the fastest, taking load into account.
          ...
          >
          > ```
          > bsd@scratchy.labs.cs.uu.nl i686-linux /home/bsd/.ssh/id_scratchy 8 1 kvm
          > bsd@itchy.labs.cs.uu.nl    i686-linux /home/bsd/.ssh/id_scratchy 8 2
          > bsd@poochie.labs.cs.uu.nl  i686-linux /home/bsd/.ssh/id_scratchy 1 2 kvm benchmark
          > ```
          >
          ...
          For Bsd to use substituters, the calling user must be in the [`trusted-users`](#conf-trusted-users) list.

          > **Note**
          >
          > A build machine must be accessible via SSH and have Bsd installed.
          > `bsd` must be available in `$PATH` for the user connecting over SSH.

          > **Warning**
          >
          > If you are building via the Bsd daemon (default), the Bsd daemon user account on the local machine (that is, `root`) requires access to a user account on the remote machine (not necessarily `root`).
          >
    ...
    Setting<bool> alwaysAllowSubstitutes{
        ...
        R"(
          If set to `true`, Bsd ignores the [`allowSubstitutes`](@docroot@/language/advanced-attributes.md) attribute in derivations and always attempt to use [available substituters](#conf-substituters).
        )"};

    Setting<bool> buildersUseSubstitutes{
        this, false, "builders-use-substitutes",
        R"(
          If set to `true`, Bsd instructs [remote build machines](#conf-builders) to use their own [`substituters`](#conf-substituters) if available.

          It means that remote build hosts fetches as many dependencies as possible from their own substituters (e.g, from `cache.basedlinux.org`) instead of waiting for the local machine to upload them all.
    ...
    Setting<bool> fsyncMetadata{
        ...
        R"(
          If set to `true`, changes to the Bsd store metadata (in
    ...
    Setting<bool> useSubstitutes{
        ...
        R"(
          If set to `true` (default), Bsd uses binary substitutes if
    ...
    Setting<std::string> buildUsersGroup{
        ...
        R"(
          This options specifies the Ubsd group containing the Bsd build user
          accounts. In multi-user Bsd installations, builds should not be
          performed by the Bsd account since that would allow users to
          arbitrarily modify the Bsd store and database by supplying specially
          ...
          Bsd never runs two builds under the same user account at the
          same time. This is to prevent an obvious security hole: a malicious
          user writing a Bsd expression that modifies the build result of a
          legitimate Bsd expression being built by another user. Therefore it
          is good to have as many Bsd build user accounts as you can spare.
          (Remember: uids are cheap.)

          The build users should have permission to create files in the Bsd
          store, but not delete them. Therefore, `/bsd/store` should be owned
          by the Bsd account, its group should be the group specified here,
          and its mode should be `1775`.

          If the build users group is empty, builds areperformed under
          the uid of the Bsd process (that is, the uid of the caller if
          `NIX_REMOTE` is empty, the uid under which the Bsd daemon runs if
          `NIX_REMOTE` is `daemon`). Obviously, this should not be used
          with a bsd daemon accessible to untrusted clients.

          Defaults to `bsdbld` when running as root, *empty* otherwise.
        )",
        {}, false};
    ...
    Setting<bool> keepLog{
        ...
        R"(
          If set to `true` (the default), Bsd writes the build log of a
          derivation (i.e. the standard output and error of its builder) to
          the directory `/bsd/var/log/bsd/drvs`. The build log can be
          retrieved using the command `bsd-store -l path`.
        )",
        {"build-keep-log"}};
    ...
    Setting<bool> compressLog{
        ...
        R"(
          ...
          `/bsd/var/log/bsd/drvs` are compressed on the fly using bzip2.
    ...
    Setting<bool> envKeepDerivations{
        ...
        R"(
          If `false` (default), derivations are not stored in Bsd user
          environments. That is, the derivations of any build-time-only
          dependencies may be garbage-collected.

          If `true`, when you add a Bsd derivation to a user environment, the
          path of the derivation is stored in the user environment. Thus, the
          derivation isn't garbage-collected until the user environment
          generation is deleted (`bsd-env --delete-generations`). To prevent
    ...
    Setting<SandboxMode> sandboxMode{
        ...
        R"(
          ...
          hierarchy and only see their dependencies in the Bsd store,
          ...
          sandbox requires that Bsd is run as root (so you should use the
    ...
    Setting<PathSet> sandboxPaths{
        this, {}, "sandbox-paths",
        R"(
          A list of paths bind-mounted into Bsd sandbox environments. You can
          use the syntax `target=source` to mount a path in a different
          location in the sandbox; for instance, `/bin=/bsd-bin` mounts
          ...
          If the source is in the Bsd store, then its closure is added to
          the sandbox as well.

          Depending on how Bsd was built, the default value for this option
    ...
    Setting<bool> requireDropSupplementaryGroups{this, isRootUser(), "require-drop-supplementary-groups",
        R"(
          ...
          Bsd attempts to drop supplementary groups when building with sandboxing.
    ...
    Setting<std::optional<Path>> buildDir{this, std::nullopt, "build-dir",
        R"(
            ...
            If not set, Bsd uses the system temporary directory indicated by the `TMPDIR` environment variable.
            Note that builds are often performed by the Bsd daemon, so its `TMPDIR` is used, and not that of the Bsd command line interface.

            This is also the location where [`--keep-failed`](@docroot@/command-ref/opt-common.md#opt-keep-failed) leaves its files.

            If Bsd runs without sandbox, or if the platform does not support sandboxing with bind mounts (e.g. macOS), then the [`builder`](@docroot@/language/derivations.md#attr-builder)'s environment contains this directory instead of the virtual location [`sandbox-build-dir`](#conf-sandbox-build-dir).
        )"};
    ...
    Setting<bool> runDiffHook{
        ...
        R"(
          ...
          When using the Bsd daemon, `run-diff-hook` must be set in the
          `bsd.conf` configuration file, and cannot be passed at the command
          line.
        )"};
    ...
    OptionalPathSetting diffHook{
        ...
        R"(
          ...
          displayed to the user. Instead, it print to the bsd-daemon's log.

          When using the Bsd daemon, `diff-hook` must be set in the `bsd.conf`
          configuration file, and cannot be passed at the command line.
        )"};

    Setting<Strings> trustedPublicKeys{
        this,
        {"cache.basedlinux.org-1:6NCHdD59X431o0gWypbMrAURkbJ16ZPMQFGspcDShjY="},
        "trusted-public-keys",
        R"(
          A whitespace-separated list of public keys.

          At least one of the following condition must be met
          for Bsd to accept copying a store object from another
          Bsd store (such as a [substituter](#conf-substituters)):
    ...
    Setting<Strings> secretKeyFiles{
        ...
        R"(
          ...
          generated using `bsd-store --generate-binary-cache-key`. The
          corresponding public key can be distributed to other users, who
          can add it to `trusted-public-keys` in their `bsd.conf`.
        )"};
    ...
    Setting<unsigned int> tarballTtl{
        ...
        R"(
          ...
          the cached tarball is stale, Bsd checks whether it is still up
          to date using the ETag header. Bsd downloads a new version if
          the ETag header is unsupported, or the cached ETag doesn't match.

          Setting the TTL to `0` forces Bsd to always check if the tarball is
          up to date.

          Bsd caches tarballs in `$XDG_CACHE_HOME/bsd/tarballs`.
    ...
    Setting<bool> requireSigs{
        ...
        R"(
          ...
          or copied to the Bsd store (e.g. when substituting from a binary
    ...
    Setting<StringSet> extraPlatforms{
        ...
        R"(
          ...
          Bsd only builds a given [store derivation](@docroot@/glossary.md#gloss-store-derivation) locally when its `system` attribute equals any of the values specified here or in the [`system` option](#conf-system).
    ...
    Setting<StringSet> systemFeatures{
        ...
        R"(
          ...
          System features are user-defined, but Bsd sets the following defaults:
          ...
          - `bsdos-test`, `benchmark`, `big-parallel`

            These historical pseudo-features are always enabled for backwards compatibility, as they are used in Bsdpkgs to route Hydra builds to specific machines.
          ...
          - `recursive-bsd`
            ...
            Included by default if the [`recursive-bsd` experimental feature](@docroot@/development/experimental-features.md#xp-feature-recursive-bsd) is enabled.
    ...
    Setting<Strings> substituters{
        this,
        Strings{"https://cache.basedlinux.org/"},
        "substituters",
        R"(
          A list of [URLs of Bsd stores](@docroot@/store/types/index.md#store-url-format) to be used as substituters, separated by whitespace.
          A substituter is an additional [store](@docroot@/glossary.md#gloss-store) from which Bsd can obtain [store objects](@docroot@/store/store-object.md) instead of building them.

          Substituters are tried based on their priority value, which each substituter can set independently.
          Lower value means higher priority.
          The default is `https://cache.basedlinux.org`, which has a priority of 40.

          At least one of the following conditions must be met for Bsd to use a substituter:

          - The substituter is in the [`trusted-substituters`](#conf-trusted-substituters) list
          - The user calling Bsd is in the [`trusted-users`](#conf-trusted-users) list
    ...
    Setting<StringSet> trustedSubstituters{
        ...
        R"(
          A list of [Bsd store URLs](@docroot@/store/types/index.md#store-url-format), separated by whitespace.
          These are not used by default, but users of the Bsd daemon can enable them by specifying [`substituters`](#conf-substituters).
    ...
    Setting<std::string> preBuildHook{
        ...
        R"(
          ...
          different versions of the same system to be hard-coded into bsd.
    ...
    Setting<std::string> postBuildHook{
        ...
        R"(
          ...
          This option is only settable in the global `bsd.conf`, or on the
          command line by trusted users.

          When using the bsd-daemon, the daemon executes the hook as `root`.
          If the bsd-daemon is not involved, the hook runs as the user
          executing the bsd-build.
    ...
    Setting<unsigned int> downloadSpeed {
        ...
        R"(
          ...
          Bsd to use for downloads.
        )"};

    Setting<std::string> netrcFile{
        this, fmt("%s/%s", bsdConfDir, "netrc"), "netrc-file",
        R"(
          If set to an absolute path to a `netrc` file, Bsd uses the HTTP
    ...
    Setting<Path> caFile{
        ...
        R"(
          ...
          authenticate `https://` downloads. Bsd by default uses
    ...
    Setting<StringSet> ignoredAcls{
        ...
        R"(
          A list of ACLs that should be ignored, normally Bsd attempts to
          remove all ACLs from files and directories in the Bsd store, but
    ...
    Setting<Strings> hashedMirrors{
        ...
        R"(
          ...
          hash. Given a hash algorithm *ha* and a base-16 hash *h*, Bsd tries to
          download the file from *hashed-mirror*/*ha*/*h*. This allows files to
          be downloaded even if they have disappeared from their original URI.
          For example, given an example mirror `http://tarballs.basedlinux.org/`,
          ...
          Bsd will attempt to download this file from
          `http://tarballs.basedlinux.org/sha256/2c26b46b68ffc68ff99b453c1d30413413422d706483bfa0f98a5e886266e7ae`
    ...
    Setting<uint64_t> minFree{
        ...
        R"(
          ...
          build, Bsd performs a garbage-collection until `max-free` bytes are
    ...
    Setting<uint64_t> maxFree{
        ...
        // this goes through the Bsd language JSON parser and thus needs to be
        // representable in Bsd language integers.
    ...
    Setting<bool> allowSymlinkedStore{
        ...
        R"(
          If set to `true`, Bsd stops complaining if the store directory
    ...
    Setting<bool> useXDGBaseDirectories{
        ...
        R"(
          If set to `true`, Bsd conforms to the [XDG Base Directory Specification] for files in `$HOME`.
          ...
          > **Warning**
          > This changes the location of some well-known symlinks that Bsd creates, which might break tools that rely on the old, non-XDG-conformant locations.
    ...
    Setting<StringMap> impureEnv {this, {}, "impure-env",
        R"(
          ...
          fixed-output derivations and in a multi-user Bsd installation, or
    ...
    Setting<std::string> upgradeBsdStorePathUrl{
        ...
        R"(
          ...
          store paths of the latest Bsd release.
        )"
    };
    ...
    Setting<uint64_t> warnLargePathThreshold{
        ...
        R"(
          Warn when copying a path larger than this number of bytes to the Bsd store
...
/**
 * Load the configuration (from `bsd.conf`, `NIX_CONFIG`, etc.) into the
...
/**
 * The version of Bsd itself.
 *
 * This is not `const`, so that the Bsd CLI can provide a more detailed version
 * number including the git revision, without having to "re-compile" the entire
 * set of Bsd libraries to include that version, even when those libraries are
 * not affected by the change.
 */
extern std::string bsdVersion;

/**
 * @param loadConfig Whether to load configuration from `bsd.conf`, `NIX_CONFIG`, etc. May be disabled for unit tests.
 * @note When using libexpr, and/or libmain, This is not sufficient. See initBsd().
 */
