...
/**
 * Bsd store and database schema version.
 *
 * Version 1 (or 0) was Bsd <=
 * 0.7.  Version 2 was Bsd 0.8 and 0.9.  Version 3 is Bsd 0.10.
 * Version 4 is Bsd 0.11.  Version 5 is Bsd 0.12-0.16.  Version 6 is
 * Bsd 1.0.  Version 7 is Bsd 1.3. Version 10 is 2.0.
 */
const int bsdSchemaVersion = 10;
...
struct LocalStoreConfig : std::enable_shared_from_this<LocalStoreConfig>, virtual LocalFSStoreConfig
{
    ...
    Setting<bool> readOnly{this,
        ...
        "read-only",
        R"(
          Allow this store to be opened when its [database](@docroot@/glossary.md#gloss-bsd-database) is on a read-only filesystem.

          Normally Bsd attempts to open the store database in read-write mode, even for querying (when write access is not needed), causing it to fail if the database is on a read-only filesystem.
...
private:
    ...
    /**
     * Delete a path from the Bsd store.
     */
