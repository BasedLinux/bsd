...
struct FileTransferSettings : Config
{
    ...
    Setting<unsigned long> stalledDownloadTimeout{
        ...
        R"(
          ...
          during download. Bsd cancels idle downloads after this
          timeout's duration.
        )"};

    Setting<unsigned int> tries{this, 5, "download-attempts",
        "The number of times Bsd attempts to download a file before giving up."};

    Setting<size_t> downloadBufferSize{this, 64 * 1024 * 1024, "download-buffer-size",
        R"(
          The size of Bsd's internal download buffer in bytes during `curl` transfers. If data is
