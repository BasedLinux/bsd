...
public:
    ...
    /**
     * Follow symlinks until we end up with a path in the Bsd store.
     */
...
public:
     ...
     * implements bsd-copy-closure's --use-substitutes
     * flag.
     */
     ...
     * (i.e. you'll get `/bsd/store/<hash>-x` rather than
     * `/bsd/store/<hash>-<name>`). Use queryPathInfo() to obtain the
...
public:
    ...
    virtual StorePath addToStoreFromDump(
        ...
        ContentAddressMethod hashMethod = ContentAddressMethod::Raw::BsdArchive,
        HashAlgorithm hashAlgo = HashAlgorithm::SHA256,
        const StorePathSet & references = StorePathSet(),
     ...
     * can be loaded into the database using `bsd-store --load-db` or
     * `bsd-store --register-validity`.
     */
    std::string makeValidityRegistration(const StorePathSet & paths,
        bool showDerivers, bool showHash);

    /**
     * Optimise the disk space usage of the Bsd store by hard-linking files
     * with the same contents.
     */
    virtual void optimiseStore() { };

    /**
     * Check the integrity of the Bsd store.
     *
    ...
    /**
     * @return An object to access files in the Bsd store.
     */
    ...
    /**
     * Export multiple paths in the format expected by ‘bsd-store
     * --import’.
     */
     ...
     * Bsd store. Optionally, the contents of the NARs are preloaded
