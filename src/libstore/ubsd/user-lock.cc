...
#include "bsd/store/user-lock.hh"
...
struct SimpleUserLock : UserLock
{
    ...
    static std::unique_ptr<UserLock> acquire()
    {
        ...
        createDirs(settings.bsdStateDir + "/userpool");
        ...
        for (auto & i : users) {
            ...
            auto fnUserLock = fmt("%s/userpool/%s", settings.bsdStateDir,pw->pw_uid);
            ...
            if (lockFile(fd.get(), ltWrite, false)) {
                ...
                if (lock->uid == getuid() || lock->uid == geteuid())
                    throw Error("the Bsd user should not be a member of '%s'", settings.buildUsersGroup);
...
struct AutoUserLock : UserLock
{
    ...
    static std::unique_ptr<UserLock> acquire(uid_t nrIds, bool useUserNamespace)
    {
        ...
        createDirs(settings.bsdStateDir + "/userpool2");
        ...
        for (size_t i = 0; i < nrSlots; i++) {
            ...
            auto fnUserLock = fmt("%s/userpool2/slot-%d", settings.bsdStateDir, i);
