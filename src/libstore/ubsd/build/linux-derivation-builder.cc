...
#  if HAVE_SECCOMP
    ...
    /* Prevent builders from using EAs or ACLs. Not all filesystems
       support these, and they're not allowed in the Bsd store because
...
struct ChrootLinuxDerivationBuilder : LinuxDerivationBuilder
{
     ...
     * paths to the sandbox as a result of recursive Bsd calls.
     */
    ...
    void prepareUser() override
    {
        if ((buildUser && buildUser->getUIDCount() != 1) || settings.useCgroups) {
            ...
            if (buildUser) {
                auto cgroupsDir = settings.bsdStateDir + "/cgroups";
    ...
    void prepareSandbox() override
    {
        /* Create a temporary directory in which we set up the chroot
           environment using bind-mounts.  We put it in the Bsd store
        ...
        /* Make the closure of the inputs available in the chroot,
           rather than the whole Bsd store.  This prevents any access
           to undeclared dependencies.  Directories are bind-mounted,
           while other inputs are hard-linked (since only directories
           can be bind-mounted).  !!! As an extra security
           precaution, make the fake Bsd store only writable by the
           build user. */
    ...
    void startChild() override
    {
        ...
        */
        ...
        writeFile(
            ...
            fmt("root:x:0:0:Bsd build user:%3%:/noshell\n"
                "bsdbld:x:%1%:%2%:Bsd build user:%3%:/noshell\n"
    ...
    void enterChroot() override
    {
        ...
        /* Bind-mount the sandbox's Bsd store onto itself so that
        ...
        if (mount(chrootStoreDir.c_str(), chrootStoreDir.c_str(), 0, MS_BIND, 0) == -1)
            throw SysError("unable to bind mount the Bsd store", chrootStoreDir);
        ...
        if (!derivationType.isSandboxed()) {
            ...
            /* N.B. it is realistic that these paths might not exist. It
               happens when testing Bsd building fixed-output derivations
...
#  endif
        ...
        /* Unshare this mount namespace. This is necessary because
           ...
           Bsd store into the sandbox. Therefore, we save the
