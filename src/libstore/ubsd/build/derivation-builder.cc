...
#include "bsd/store/build/child.hh"
 ...
 */
// FIXME: rename this to UbsdDerivationBuilder or something like that.
...
protected:
    ...
    /**
     * The recursive Bsd daemon socket.
     */
...
protected:
     ...
     * This includes:
     *
     * * The paths specified by the `sandbox-paths` setting, and their closure in the Bsd store.
...
private:
    ...
    /**
     * Start an in-process bsd daemon thread for recursive-bsd.
     */
    void startDaemon();
...
std::variant<std::pair<BuildResult::Status, Error>, SingleDrvOutputs> DerivationBuilderImpl::unprepareBuild()
{
    ...
    /* Terminate the recursive Bsd daemon. */
    stopDaemon();
...
void DerivationBuilderImpl::startBuilder()
{
    ...
    if (!drvOptions.canBuildLocally(store, drv)) {
        ...
        if (drv.platform == "x86_64-darwin" && settings.thisSystem == "aarch64-darwin")
          msg += fmt("\nNote: run `%s` to run programs for x86_64-darwin", Magenta("/usr/sbin/softwareupdate --install-rosetta && launchctl stop org.bsdos.bsd-daemon"));
    ...
    /* Create a temporary directory where the build will take
       place. */
    topTmpDir = createTempDir(settings.buildDir.get().value_or(""), "bsd-build-" + std::string(drvPath.name()), 0700);
    setBuildTmpDir();
    assert(!tmpDir.empty());
    ...
    /* Fire up a Bsd daemon to process recursive Bsd calls from the
       builder. */
    if (drvOptions.getRequiredSystemFeatures(drv).count("recursive-bsd"))
        startDaemon();
...
void DerivationBuilderImpl::initEnv()
{
    ...
    /* Tell the builder where the Bsd store is.  Usually they
    ...
    if (!parsedDrv) {
        for (auto & i : drv.env) {
            if (drvOptions.passAsFile.find(i.first) == drvOptions.passAsFile.end()) {
                ...
                std::string fn = ".attr-" + hash.to_string(HashFormat::Bsd32, false);
                Path p = tmpDir + "/" + fn;
    ...
    /* Compatibility hack with Bsd <= 0.7: if this is a fixed-output
       derivation, tell the builder, so that for instance `fetchurl'
       can skip checking the output.  On older Bsdes, this environment
...
{
    experimentalFeatureSettings.require(Xp::RecursiveBsd);
    ...
    auto socketName = ".bsd-socket";
    Path socketPath = tmpDir + "/" + socketName;
    env["NIX_REMOTE"] = "ubsd://" + tmpDirInSandbox() + "/" + socketName;

    daemonSocket = createUbsdDomainSocket(socketPath, 0600);
        ...
        auto newInfoFromCA = [&](const DerivationOutput::CAFloating outputHash) -> ValidPathInfo {
...

            {
                HashResult narHashAndSize = hashPath(
                    {getFSSourceAccessor(), CanonPath(actualPath)},
                    FileSerialisationMethod::BsdArchive, HashAlgorithm::SHA256);
