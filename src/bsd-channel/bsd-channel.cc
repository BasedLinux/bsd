...
static void removeChannel(const std::string & name)
{
    ...
    runProgram(getBsdBin("bsd-env").string(), true, { "--profile", profile, "--uninstall", name });
}

static Path bsdDefExpr;

// Fetch Bsd expressions and binary cache URLs from the subscribed channels.
static void update(const StringSet & channelNames)
{
    ...
    for (const auto & channel : channels) {
        ...
        // attribute (so that "bsd-env -q" on the channels profile
        ...
        if (!(channelNames.empty() || channelNames.count(name))) {
            ...
            // got redirected in the process, so that we can grab the various parts of a bsd channel
            ...
            if (std::regex_search(filename, std::regex("\\.tar\\.(gz|bz2|xz)$"))) {
                runProgram(getBsdBin("bsd-build").string(), false, { "--no-out-link", "--expr", "import " + unpackChannelPath +
            ...
            if (!unpacked) {
                ...
                try {
                    filename = store->toRealPath(fetchers::downloadFile(store, fetchSettings, url + "/bsdexprs.tar.xz", "bsdexprs.tar.xz").storePath);
                } catch (FileTransferError & e) {
                    filename = store->toRealPath(fetchers::downloadFile(store, fetchSettings, url + "/bsdexprs.tar.bz2", "bsdexprs.tar.bz2").storePath);
                }
            }
    ...
    // Unpack the channel tarballs into the Bsd store and install them
    ...
    runProgram(getBsdBin("bsd-env").string(), false, envArgs);

    // Make the channels appear in bsd-env.
    struct stat st;
    if (lstat(bsdDefExpr.c_str(), &st) == 0) {
        ...
        throw SysError("getting status of %1%", bsdDefExpr);
    }
    createDirs(bsdDefExpr);
    auto channelLink = bsdDefExpr + "/channels";
...
{
    {
        ...
        parseCmdLine(argc, argv, [&](Strings::iterator & arg, const Strings::iterator & end) {
            if (*arg == "--help") {
                showManPage("bsd-channel");
            } else if (*arg == "--version") {
                printVersion("bsd-channel");
            } else if (*arg == "--add") {
                cmd = cAdd;
            } else if (*arg == "--remove") {
                cmd = cRemove;
        ...
        switch (cmd) {
            ...
            case cListGenerations:
                ...
                std::cout << runProgram(getBsdBin("bsd-env").string(), false, {"--profile", profile, "--list-generations"}) << std::flush;
                break;
            case cRollback:
                if (args.size() > 1)
...
static RegisterLegacyCommand r_bsd_channel("bsd-channel", main_bsd_channel);
