...
static int main_bsd_collect_garbage(int argc, char * * argv)
{
    {
        ...
        parseCmdLine(argc, argv, [&](Strings::iterator & arg, const Strings::iterator & end) {
            if (*arg == "--help")
                showManPage("bsd-collect-garbage");
            else if (*arg == "--version")
                printVersion("bsd-collect-garbage");
        ...
        if (removeOld) {
            std::set<std::filesystem::path> dirsToClean = {
                profilesDir(),
                std::filesystem::path{settings.bsdStateDir} / "profiles",
...
static RegisterLegacyCommand r_bsd_collect_garbage("bsd-collect-garbage", main_bsd_collect_garbage);
