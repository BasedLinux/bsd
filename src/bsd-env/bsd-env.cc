...
#include "bsd/expr/eval-settings.hh" // for defexpr
...
struct InstallSourceInfo
{
    ...
    std::shared_ptr<SourcePath> bsdExprPath; /* for srcBsdExprDrvs, srcBsdExprs */
    Path profile; /* for srcProfile */
    std::string systemFilter; /* for srcBsdExprDrvs */
    Bindings * autoArgs;
};
    ...
    Strings::iterator & i, Strings & args, const std::string & arg)
{
    if (arg == "--from-expression" || arg == "-E")
        globals.instSource.type = srcBsdExprs;
...
static bool isBsdExpr(const SourcePath & path, struct SourceAccessor::Stat & st)
{
    return
    ...
    const SourcePath & path, StringSet & seen, BindingsBuilder & attrs)
{
    ...
    for (auto & i : namesSorted) {
        ...
        if (isBsdExpr(path2, st) && (st.type != SourceAccessor::tRegular || hasSuffix(path2.baseName(), ".bsd"))) {
            ...
            if (!seen.insert(attrName).second) {
                ...
                printError("warning: name collision in input Bsd expressions, skipping '%1%'"
            ...
            if (seen.size() == maxAttrs)
                throw Error("too many Bsd expressions in directory '%1%'", path);
...
static void loadSourceExpr(EvalState & state, const SourcePath & path, Value & v)
{
    ...
    if (isBsdExpr(path, st))
        state.evalFile(path, v);

    /* The path is a directory.  Put the Bsd expressions in the
    ...
    else throw Error("path '%s' is not a directory or a Bsd expression", path);
}


static void loadDerivations(EvalState & state, const SourcePath & bsdExprPath,
    std::string systemFilter, Bindings & autoArgs,
    const std::string & pathPrefix, PackageInfos & elems)
{
    Value vRoot;
    loadSourceExpr(state, bsdExprPath, vRoot);
...
static BsdInt getPriority(EvalState & state, PackageInfo & drv)
{
    return drv.queryMetaInt("priority", BsdInt(0));
}
    ...
    PackageInfos & elems, bool newestOnly)
{
    ...
    switch (type) {
        ...
        /* Get the available user environment elements from the
           derivations specified in a Bsd expression, including only
        ...
        case srcBsdExprDrvs: {
            ...
            loadDerivations(state, *instSource.bsdExprPath,
        ...
        case srcBsdExprs: {
            ...
            Value vArg;
            loadSourceExpr(state, *instSource.bsdExprPath, vArg);
        ...
        case srcAttrPath: {
            Value vRoot;
            loadSourceExpr(state, *instSource.bsdExprPath, vRoot);
            for (auto & i : args) {
    ...
    const Strings & args, UpgradeType upgradeType)
{
    ...
    /* Upgrade works as follows: we take all currently installed
       ...
       for a derivation in the input Bsd expression that has the same
    ...
    while (true) {
        ...
        for (auto & i : installedElems) {
            ...
            try {
                ...
                /* Find the derivation in the input Bsd expression
...
static void opSet(Globals & globals, Strings opFlags, Strings opArgs)
{
    ...
    if (!store2) throw Error("--set is not supported for this Bsd store");
...
static void opQuery(Globals & globals, Strings opFlags, Strings opArgs)
{
    ...
    if (source == sAvailable || compareVersions)
        loadDerivations(*globals.state, *globals.instSource.bsdExprPath,
...
{
    printVersion("bsd-env");
}


static int main_bsd_env(int argc, char * * argv)
{
    {
        ...
        Path bsdExprPath = getBsdDefExpr();

        if (!pathExists(bsdExprPath)) {
            try {
                createDirs(bsdExprPath);
                replaceSymlink(
                    defaultChannelsDir(),
                    bsdExprPath + "/channels");
                if (!isRootUser())
                    replaceSymlink(
                        rootChannelsDir(),
                        bsdExprPath + "/channels_root");
            } catch (Error &) { }
        }
        ...
        MyArgs myArgs(std::string(baseNameOf(argv[0])), [&](Strings::iterator & arg, const Strings::iterator & end) {
            ...
            else if (*arg == "--force-name") // undocumented flag for bsd-install-package
        ...
        if (showHelp) showManPage("bsd-env" + opName);
        ...
        globals.instSource.bsdExprPath = std::make_shared<SourcePath>(
            file != ""
            ? lookupFileArg(*globals.state, file)
            : globals.state->rootPath(CanonPath(bsdExprPath)));
...
static RegisterLegacyCommand r_bsd_env("bsd-env", main_bsd_env);
