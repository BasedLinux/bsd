...
static constexpr Command::Category catBsdInstallation = 102;

static constexpr auto installablesCategory =
    "Options that change the interpretation of [installables](@docroot@/command-ref/new-cli/bsd.md#installables)";

struct BsdMultiCommand : MultiCommand, virtual Command
{
...
/**
 * A command that requires a \ref Store "Bsd store".
 */
struct StoreCommand : virtual Command
{
    ...
    /**
     * Return the default Bsd store.
     */
    ref<Store> getStore();

    /**
     * Return the destination Bsd store.
     */
...
/**
 * A command that needs to evaluate Bsd language expressions.
 */
 ...
 * store paths, attribute paths, Bsd expressions, etc.
 */
...
struct RegisterCommand
{
    ...
    static bsd::Commands getCommandsFor(const std::vector<std::string> & prefix);
};
