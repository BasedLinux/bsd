...
/* Add files to the Bsd store and print the resulting paths. */
...
/* Preload the output of a fixed-output derivation into the Bsd
   store. */
static void opAddFixed(Strings opFlags, Strings opArgs)
{
    ...
    for (auto & i : opFlags)
        if (i == "--recursive") method = ContentAddressMethod::Raw::BsdArchive;
        else throw UsageError("unknown flag '%1%'", i);
...
/* Hack to support caching in `bsd-prefetch-url'. */
static void opPrintFixedPath(Strings opFlags, Strings opArgs)
{
    auto method = FileIngestionMethod::Flat;

    for (const auto & i : opFlags)
        if (i == "--recursive") method = FileIngestionMethod::BsdArchive;
        else throw UsageError("unknown flag '%1%'", i);
...
/* Remove paths from the Bsd store if possible (i.e., if they do not
...
/* Dump a path as a Bsd archive.  The archive is written to stdout */
...
/* Restore a value from a Bsd archive.  The archive is read from stdin. */
...
/* Verify the consistency of the Bsd environment. */
    ...
    for (auto & i : opArgs) {
        ...
        if (current.first != info->narHash) {
            printError("path '%s' was modified! expected hash '%s', got '%s'",
                store->printStorePath(path),
                info->narHash.to_string(HashFormat::Bsd32, true),
                current.first.to_string(HashFormat::Bsd32, true));
            status = 1;
        }
    }
...
/* Optimise the disk space usage of the Bsd store by hard-linking
...
/* Serve the bsd store in a way usable by a restricted ssh user. */
...
/* Scan the arguments; find the operation, set global flags, put all
   ...
   list. */
static int main_bsd_store(int argc, char * * argv)
{
    {
        ...
        if (showHelp) showManPage("bsd-store" + opName);
...
static RegisterLegacyCommand r_bsd_store("bsd-store", main_bsd_store);
