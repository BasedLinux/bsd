...
/**
 * A documentation comment, in the sense of [RFC 145](https://github.com/BasedLinux/rfcs/blob/master/rfcs/0145-doc-strings.md)
 *
...
/* Abstract syntax of Bsd expressions. */

struct Expr
{
    struct AstSymbols {
        Symbol sub, lessThan, mul, div, or_, findFile, bsdPath, body;
    };
...
{
    Value v;
    ExprInt(BsdInt n) { v.mkInt(n); };
    ExprInt(BsdInt::Inner n) { v.mkInt(n); };
...
{
    Value v;
    ExprFloat(BsdFloat nf) { v.mkFloat(nf); };
    Value * maybeThunk(EvalState & state, Env & env) override;
    COMMON_METHODS
};
...
struct ExprCall : Expr
{
    ...
    PosIdx pos;
    std::optional<PosIdx> cursedOrEndPos; // used during parsing to warn about https://github.com/BasedLinux/bsd/issues/11118
