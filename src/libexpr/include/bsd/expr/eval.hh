...
#include "bsd/expr/repl-exit-status.hh"
...
void copyContext(const Value & v, BsdStringContext & context, const ExperimentalFeatureSettings & xpSettings = experimentalFeatureSettings);
...
public:
    ...
    /**
     * If set, force copying files to the Bsd store even if they
     * already exist there.
     */
    RepairFlag repair;
    ...
    /**
     * The in-memory filesystem for <bsd/...> paths.
     */
...
public:
    ...
    /**
     * Parse a Bsd expression from the specified file.
     */
    Expr * parseExprFromFile(const SourcePath & path);
    Expr * parseExprFromFile(const SourcePath & path, std::shared_ptr<StaticEnv> & staticEnv);

    /**
     * Parse a Bsd expression from the specified string.
     */
     ...
     */
    BsdInt forceInt(Value & v, const PosIdx pos, std::string_view errorCtx);
    BsdFloat forceFloat(Value & v, const PosIdx pos, std::string_view errorCtx);
    ...
    std::string_view forceString(Value & v, BsdStringContext & context, const PosIdx pos, std::string_view errorCtx, const ExperimentalFeatureSettings & xpSettings = experimentalFeatureSettings);
...
public:
    ...
    std::optional<std::string> tryAttrsToString(const PosIdx pos, Value & v,
        BsdStringContext & context, bool coerceMore = false, bool copyToStore = true);
     ...
     * referenced paths are copied to the Bsd store as a side effect.
     */
    BackedStringView coerceToString(const PosIdx pos, Value & v, BsdStringContext & context,
        std::string_view errorCtx,
        bool coerceMore = false, bool copyToStore = true,
        bool canonicalizePath = true);

    StorePath copyPathToStore(BsdStringContext & context, const SourcePath & path);
     ...
     */
    SourcePath coerceToPath(const PosIdx pos, Value & v, BsdStringContext & context, std::string_view errorCtx);

    /**
     * Like coerceToPath, but the result must be a store path.
     */
    StorePath coerceToStorePath(const PosIdx pos, Value & v, BsdStringContext & context, std::string_view errorCtx);
     ...
     * element, which is either a `BsdStringContextElem::Opaque` or
     * `BsdStringContextElem::Built`. (`BsdStringContextEleme::DrvDeep`
     * is not permitted).
     *
...
private:
    ...
    /**
     * Current Bsd call stack depth, used with `max-call-depth` setting to throw stack overflow hopefully before we run out of system stack.
     */
    size_t callDepth = 0;
...
public:
     ...
     * single `BsdStringContextElem::Opaque` element of that store path.
     */
     ...
     *
     * The string is the printed store path with a context containing a
     * single `BsdStringContextElem::Built` element of the drv path and
     * output name.
     *
     ...
     *
     * @return true if Bsd was built with GC and a GC was performed, false if not.
     ...
     */
    [[nodiscard]] StringMap realiseContext(const BsdStringContext & context, StorePathSet * maybePaths = nullptr, bool isIFD = true);
