...
struct EvalSettings : Config
{
    ...
    static Strings getDefaultBsdPath();

    static bool isPseudoUrl(std::string_view s);

    static Strings parseBsdPath(const std::string & s);
    ...
    Setting<bool> enableNativeCode{this, false, "allow-unsafe-native-code-during-evaluation", R"(
        ...
        - `builtins.importNative` *path* *symbol*
          ...
          The [Bsd C++ API documentation](@docroot@/development/documentation.md#api-documentation) has more details on evaluator internals.

        - `builtins.exec` *arguments*

          Execute a program, where *arguments* are specified as a list of strings, and parse its output as a Bsd expression.
    )"};

    Setting<Strings> bsdPath{
        this, {}, "bsd-path",
        R"(
          ...
          - `NIX_PATH` is overridden by [specifying the setting as the command line flag](@docroot@/command-ref/conf-file.md#command-line-flags) `--bsd-path`.
          - Any current value is extended by the [`-I` option](@docroot@/command-ref/opt-common.md#opt-I) or `--extra-bsd-path`.
          ...
          - `bsdpkgs=$NIX_STATE_DIR/profiles/per-user/root/channels/bsdpkgs`

            The current state of the `bsdpkgs` channel for the `root` user.

          - `$NIX_STATE_DIR/profiles/per-user/root/channels`

            The current state of all channels for the `root` user.

          These files are set up by the [Bsd installer](@docroot@/installation/installing-binary.md).
    ...
    Setting<std::string> currentSystem{
        ...
        R"(
          ...
          in the Bsd language if it is set as a non-empty string.
          ...
          This is useful for evaluating Bsd code on one system to produce derivations to be built on another type of system.
        )"};
    ...
    Setting<bool> pureEval{this, false, "pure-eval",
        R"(
          Pure evaluation mode ensures that the result of Bsd expressions is fully determined by explicitly declared inputs, and not influenced by external state:
    ...
    Setting<bool> traceImportFromDerivation{
        ...
        R"(
          By default, Bsd allows [Import from Derivation](@docroot@/language/import-from-derivation.md).

          When this setting is `true`, Bsd logs a warning indicating that it performed such an import.
    ...
    Setting<bool> enableImportFromDerivation{
        ...
        R"(
          ...
          With this option set to `false`, Bsd throws an error when evaluating an expression that uses this feature,
    ...
    Setting<Strings> allowedUris{this, {}, "allowed-uris",
        R"(
          ...
          `https://github.com/BasedLinux`, builtin functions such as `fetchGit` are
          allowed to access `https://github.com/BasedLinux/patchelf.git`.
    ...
    Setting<bool> traceFunctionCalls{this, false, "trace-function-calls",
        R"(
          If set to `true`, the Bsd evaluator traces every function call.
          Bsd prints a log message at the "vomit" level for every function
          ...
          Use the `contrib/stack-collapse.py` script distributed with the Bsd
    ...
    Setting<Path> evalProfileFile{this, "bsd.profile", "eval-profile-file",
        R"(
    ...
    Setting<bool> ignoreExceptionsDuringTry{this, false, "ignore-try",
        R"(
          If set to true, ignore exceptions inside 'tryEval' calls when evaluating Bsd expressions in
    ...
    Setting<bool> builtinsTraceDebugger{this, false, "debugger-on-trace",
        R"(
          ...
          * [`builtins.warn`](@docroot@/language/builtins.md#builtins-warn)
          ...
          This is useful for debugging warnings in third-party Bsd code.
        )"};
    ...
    Setting<bool> builtinsAbortOnWarn{this, false, "abort-on-warn",
        R"(
          ...
          This is useful for finding information about warnings in third-party Bsd code when you can not start the interactive debugger, such as when Bsd is called from a non-interactive script. See [`debugger-on-warn`](#conf-debugger-on-warn).
...
/**
 * Conventionally part of the default bsd path in impure mode.
 */
Path getBsdDefExpr();
