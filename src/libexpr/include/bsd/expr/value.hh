...
using BsdInt = checked::Checked<int64_t>;
using BsdFloat = double;

/**
 * External values must descend from ExternalValueBase, so that
 * type-agnostic bsd functions (e.g. showType) can be implemented
 */
class ExternalValueBase
{
     ...
     * error.
     */
    virtual std::string coerceToString(EvalState & state, const PosIdx & pos, BsdStringContext & context, bool copyMore, bool copyToStore) const;
    ...
    virtual nlohmann::json printValueAsJSON(EvalState & state, bool strict,
        BsdStringContext & context, bool copyToStore = true) const;

    /**
     * Print the value as XML. Defaults to unevaluated
     */
    virtual void printValueAsXML(EvalState & state, bool strict, bool location,
        XMLWriter & doc, BsdStringContext & context, PathSet & drvsSeen,
...
public:
    ...
    inline void mkInt(BsdInt::Inner n)
    {
        mkInt(BsdInt{n});
    }

    inline void mkInt(BsdInt n)
    {
    ...
    void mkString(std::string_view s, const BsdStringContext & context);

    void mkStringMove(const char * s, const BsdStringContext & context);
    ...
    inline void mkFloat(BsdFloat n)
    {
    ...
    BsdInt integer() const
    { return payload.integer; }

    BsdFloat fpoint() const
