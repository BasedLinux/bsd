...
struct LockFlags
{
     ...
     * get a fatal error. If set to false, Bsd will use the modified
     ...
     * references like 'bsdpkgs'.
     */
    std::optional<bool> useRegistries = std::nullopt;

    /**
     * Whether to apply flake's bsdConfig attribute to the configuration
     */

    bool applyBsdConfig = false;
