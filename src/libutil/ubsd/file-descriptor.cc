...
#include "util-ubsd-config-private.hh"
...
// TODO: get rid of buildhook and remove this function again (https://github.com/BasedLinux/bsd/issues/12688)
...
void Pipe::create()
{
    ...
    ubsd::closeOnExec(fds[0]);
    ubsd::closeOnExec(fds[1]);
#endif
    readSide = fds[0];
    writeSide = fds[1];
}
...
static int ubsd_close_range(unsigned int first, unsigned int last, int flags)
{
#if !HAVE_CLOSE_RANGE
...
void ubsd::closeExtraFDs()
{
    ...
    if (ubsd_close_range(MAX_KEPT_FD + 1, ~0U, 0) == 0) {
        return;
    }
#endif
