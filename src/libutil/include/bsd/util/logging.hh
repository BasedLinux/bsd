...
struct LoggerSettings : Config
{
    Setting<bool> showTrace{
        ...
        R"(
          Whether Bsd should print out a stack trace in case of Bsd
    ...
    Setting<Path> jsonLogPath{
        this, "", "json-log-path",
        R"(
          A file or ubsd socket to which JSON records of Bsd's log output are
          written, in the same format as `--log-format internal-json`
          (without the `@bsd ` prefixes on each line).
          Concurrent writes to the same file by multiple Bsd processes are not supported and
...
std::unique_ptr<Logger> makeJSONLogger(Descriptor fd, bool includeBsdPrefix = true);

std::unique_ptr<Logger> makeJSONLogger(const std::filesystem::path & path, bool includeBsdPrefix = true);
