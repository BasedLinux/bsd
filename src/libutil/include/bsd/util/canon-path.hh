 ...
 *
 * `CanonPath` are "virtual" Bsd paths for abstract file system objects;
 * they are always Ubsd-style paths, regardless of what OS Bsd is
 ...
 * Ubsd. `"foo/bar"` is a relative path because an absolute path would
 * "override" the `some_fd` directory file descriptor and escape to the
 * "system root". Conversely, Bsd's abstract file operations *never* escape the
...
template<>
struct std::hash<bsd::CanonPath>
{
    std::size_t operator ()(const bsd::CanonPath & s) const noexcept
    {
