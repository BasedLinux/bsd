...
struct DevelopSettings : Config
{
    Setting<std::string> bashPrompt{this, "", "bash-prompt",
        "The bash prompt (`PS1`) in `bsd develop` shells."};

    Setting<std::string> bashPromptPrefix{this, "", "bash-prompt-prefix",
        "Prefix prepended to the `PS1` environment variable in `bsd develop` shells."};

    Setting<std::string> bashPromptSuffix{this, "", "bash-prompt-suffix",
        "Suffix appended to the `PS1` environment variable in `bsd develop` shells."};
};
...
static StorePath getDerivationEnvironment(ref<Store> store, ref<Store> evalStore, const StorePath & drvPath)
{
    ...
    if (builder != "bash")
        throw Error("'bsd develop' only works on derivations that use 'bash' as their builder");
...
struct Common : InstallableCommand, MixProfile
{
        ...
        const std::filesystem::path & outputsDir = std::filesystem::path { std::filesystem::current_path() } / "outputs")
    {
        ...
        for (auto & var : savedVars) {
            out << fmt("%s=${%s:-}\n", var, var);
            out << fmt("bsd_saved_%s=\"$%s\"\n", var, var);
        }

        buildEnvironment.toBash(out, ignoreVars);

        for (auto & var : savedVars)
            out << fmt("%s=\"$%s${bsd_saved_%s:+:$bsd_saved_%s}\"\n", var, var, var, var);

        out << "export NIX_BUILD_TOP=\"$(mktemp -d -t bsd-shell.XXXXXX)\"\n";
...
struct CmdDevelop : Common, MixEnvironment
{
    ...
    void run(ref<Store> store, ref<Installable> installable) override
    {
        ...
        auto [rcFileFd, rcFilePath] = createTempFile("bsd-shell");

        AutoDelete tmpDir(createTempDir("", "bsd-develop"), true);
        ...
        try {
            ...
            auto bsdpkgsLockFlags = lockFlags;
            bsdpkgsLockFlags.inputOverrides = {};
            bsdpkgsLockFlags.inputUpdates = {};

            auto bsdpkgs = defaultBsdpkgsFlakeRef();
            if (auto * i = dynamic_cast<const InstallableFlake *>(&*installable))
                bsdpkgs = i->bsdpkgsFlakeRef();
            ...
            if (!found)
                throw Error("package 'bsdpkgs#bashInteractive' does not provide a 'bin/bash'");
...
struct CmdPrintDevEnv : Common, MixJSON
{
    ...
    void run(ref<Store> store, ref<Installable> installable) override
    {
        ...
        if (json) {
            ...
            AutoDelete tmpDir(createTempDir("", "bsd-dev-env"), true);
