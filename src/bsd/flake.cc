...
public:
            ...
            .handler={[&](std::vector<std::string> inputsToUpdate){
                for (const auto & inputToUpdate : inputsToUpdate) {
                    ...
                    try {
                        ...
                        warn("Invalid flake input '%s'. To update a specific flake, use 'bsd flake update --flake %s' instead.", inputToUpdate, inputToUpdate);
                        throw e;
                    }
    ...
    void run(bsd::ref<bsd::Store> store) override
    {
...
struct CmdFlakeMetadata : FlakeCommand, MixJSON
{
    ...
    void run(bsd::ref<bsd::Store> store) override
    {
        ...
        // Currently, all flakes are in the Bsd store via the rootFS accessor.
...
struct CmdFlakeInfo : CmdFlakeMetadata
{
    void run(bsd::ref<bsd::Store> store) override
    {
        warn("'bsd flake info' is a deprecated alias for 'bsd flake metadata'");
    ...
    void run(bsd::ref<bsd::Store> store) override
    {
        ...
        auto checkOverlay = [&](std::string_view attrPath, Value & v, const PosIdx pos) {
            try {
                ...
                // FIXME: if we have a 'bsdpkgs' input, use it to
        ...
        auto checkModule = [&](std::string_view attrPath, Value & v, const PosIdx pos) {
            try {
                Activity act(*logger, lvlInfo, actUnknown,
                    fmt("checking BasedLinux module '%s'", attrPath));
                state->forceValue(v, pos);
            } catch (Error & e) {
                e.addTrace(resolve(pos), HintFmt("while checking the BasedLinux module '%s'", attrPath));
                reportError(e);
            }
        };
        ...
        auto checkBasedLinuxConfiguration = [&](const std::string & attrPath, Value & v, const PosIdx pos) {
            try {
                Activity act(*logger, lvlInfo, actUnknown,
                    fmt("checking BasedLinux configuration '%s'", attrPath));
                ...
                e.addTrace(resolve(pos), HintFmt("while checking the BasedLinux configuration '%s'", attrPath));
                reportError(e);
            }
        };
                ...
                [&](std::string_view name, Value & vOutput, const PosIdx pos) {
                    ...
                    try {
                        ...
                        std::string_view replacement =
                            ...
                            name == "bsdosModule" ? "bsdosModules.default" :
                            "";
                        if (replacement != "")
                        ...
                        else if (name == "bsdosModule")
                            checkModule(name, vOutput, pos);

                        else if (name == "bsdosModules") {
                            state->forceAttrs(vOutput, pos, "");
                            for (auto & attr : *vOutput.attrs())
                        ...
                        else if (name == "bsdosConfigurations") {
                            state->forceAttrs(vOutput, pos, "");
                            for (auto & attr : *vOutput.attrs())
                                checkBasedLinuxConfiguration(fmt("%s.%s", name, state->symbols[attr.name]),
                                    *attr.value, attr.pos);
                        }
        ...
        if (!omittedSystems.empty()) {
            // TODO: empty system is not visible; render all as bsd strings?
            warn(
...
struct CmdFlakeArchive : FlakeCommand, MixJSON, MixDryRun
{
    ...
    CmdFlakeArchive()
    {
        addFlag({
            .longName = "to",
            .description = "URI of the destination Bsd store",
            .labels = {"store-uri"},
            .handler = {&dstUri},
        });
        addFlag({
            ...
            try {
                ...
                if ((attrPathS.size() == 1)
                    && (attrPathS[0] == "formatter"
                        || attrPathS[0] == "bsdosConfigurations"
                        || attrPathS[0] == "bsdosModules"
                ...
                // bsdpkgs. We still want to recurse into it, instead of
                // skipping it at all.
                return true;
            }
        };
            ...
            try {
                    ...
                    (attrPath.size() == 2 && attrPathS[0] == "templates"))
                {
                    ...
                    auto [type, description] =
                        ...
                        || (attrPath.size() == 2 && attrPathS[0] == "overlays") ? std::make_pair("bsdpkgs-overlay", "Bsdpkgs overlay") :
                        attrPath.size() == 2 && attrPathS[0] == "bsdosConfigurations" ? std::make_pair("bsdos-configuration", "BasedLinux configuration") :
                        (attrPath.size() == 1 && attrPathS[0] == "bsdosModule")
                        || (attrPath.size() == 2 && attrPathS[0] == "bsdosModules") ? std::make_pair("bsdos-module", "BasedLinux module") :
...
struct CmdFlakePrefetch : FlakeCommand, MixJSON
{
    ...
    std::string description() override
    {
        return "download the source tree denoted by a flake reference into the Bsd store";
    }
    ...
    void run(ref<Store> store) override
    {
        ...
        if (outLink) {
            if (auto store2 = store.dynamic_pointer_cast<LocalFSStore>())
                ...
                throw Error("'--out-link' is not supported for this Bsd store");
        }
    }
};

struct CmdFlake : BsdMultiCommand
{
    CmdFlake()
        : BsdMultiCommand(
            "flake",
