...
struct CmdEnv : BsdMultiCommand
{
    CmdEnv()
        : BsdMultiCommand("env", RegisterCommand::getCommandsFor({"env"}))
    {
    }
    ...
    void run(ref<Store> store, Installables && installables) override
    {
        ...
        while (!todo.empty()) {
            ...
            if (!store->isInStore(binDir.abs()))
                throw Error("path '%s' is not in the Bsd store", binDir);

            pathAdditions.push_back(binDir.abs());

            auto propPath = state->storeFS->resolveSymlinks(
                CanonPath(store->printStorePath(path)) / "bsd-support" / "propagated-user-env-packages");
        ...
        auto ubsdPath = ExecutablePath::load();
        ubsdPath.directories.insert(ubsdPath.directories.begin(), pathAdditions.begin(), pathAdditions.end());
        auto ubsdPathString = ubsdPath.render();
        setEnvOs(OS_STR("PATH"), ubsdPathString.c_str());
