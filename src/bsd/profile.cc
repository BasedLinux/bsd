...
struct ProfileManifest
{
    ...
    ProfileManifest(EvalState & state, const std::filesystem::path & profile)
    {
        ...
        else if (std::filesystem::exists(profile / "manifest.bsd")) {
            // FIXME: needed because of pure mode; ugly.
            state.allowPath(state.store->followLinksToStore(profile.string()));
            state.allowPath(state.store->followLinksToStore((profile / "manifest.bsd").string()));
    ...
    nlohmann::json toJSON(Store & store) const
    {
        ...
        // like in https://github.com/BasedLinux/bsd/issues/10109
        json["version"] = 3;
        json["elements"] = es;
        return json;
    }
...
struct CmdProfileAdd : InstallablesCommand, MixDefaultProfile
{
    ...
    void run(ref<Store> store, Installables && installables) override
    {
        ...
        try {
            ...
            //       See https://github.com/BasedLinux/bsd/compare/3efa476c5439f8f6c1968a6ba20a31d1239c2f04..1fe5d172ece51a619e879c4b86f603d9495cc102
...
{
    bsd::StorePath storePath;

    StorePathMatcher(const bsd::StorePath & storePath) : storePath(storePath)
    { }
            ...
            .handler = {[this](std::vector<std::string> args) {
                for (auto & arg : args) {
                    if (auto n = string2Int<size_t>(arg)) {
                        throw Error("'bsd profile' no longer supports indices ('%d')", *n);
...
struct CmdProfileRemove : virtual EvalCommand, MixDefaultProfile, MixProfileElementMatchers
{
    ...
    void run(ref<Store> store) override
    {
        ...
        if (matchingElementNames.empty()) {
            warn ("No packages to remove. Use 'bsd profile list' to see the current profile.");
            return;
        }
...
struct CmdProfileUpgrade : virtual SourceExprCommand, MixDefaultProfile, MixProfileElementMatchers
{
    ...
    void run(ref<Store> store) override
    {
        ...
        if (matchingElementNames.empty()) {
            warn("No packages to upgrade. Use 'bsd profile list' to see the current profile.");
            return;
        }
...
struct CmdProfile : BsdMultiCommand
{
    CmdProfile()
        : BsdMultiCommand(
            "profile",
