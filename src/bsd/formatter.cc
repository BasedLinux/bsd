...
struct CmdFormatter : BsdMultiCommand
{
    CmdFormatter()
        : BsdMultiCommand("formatter", RegisterCommand::getCommandsFor({"formatter"}))
    {
    }
...
/** Common implementation bits for the `bsd formatter` subcommands. */
