...
using namespace bsd;
using namespace bsd::daemon;

/**
 * Settings related to authenticating clients for the Bsd daemon.
 *
 * For pipes we have little good information about the client side, but
 * for Ubsd domain sockets we do. So currently these options implemented
...
struct AuthorizationSettings : Config {
    ...
    Setting<Strings> trustedUsers{
        ...
        R"(
          ...
          These users will have additional rights when connecting to the Bsd daemon, such as the ability to specify additional [substituters](#conf-substituters), or to import unsigned realisations or unsigned input-addressed store objects.
    ...
    Setting<Strings> allowedUsers{
        ...
        R"(
          ...
          These users are allowed to connect to the Bsd daemon.
          ...
          > **Note**
          >
          > Trusted users (set in [`trusted-users`](#conf-trusted-users)) can always connect to the Bsd daemon.
        )"};
};
...
static std::pair<TrustedFlag, std::string> authPeer(const PeerInfo & peer)
{
    ...
    if ((!trusted && !matchUser(user, group, allowedUsers)) || group == settings.buildUsersGroup)
        throw Error("user '%1%' is not allowed to connect to the Bsd daemon", user);
 ...
 * and user credentials (from the ubsd domain socket).
 */
static void daemonLoop(std::optional<TrustedFlag> forceTrustClientOpt)
{
    ...
    if (listenFds) {
        ...
        ubsd::closeOnExec(fdSocket.get());
    }

    //  Otherwise, create and bind to a Ubsd domain socket.
    else {
        createDirs(dirOf(settings.bsdDaemonSocketFile));
        fdSocket = createUbsdDomainSocket(settings.bsdDaemonSocketFile, 0666);
    }
    ...
    while (1) {
        ...
        try {
            ...
            options.errorPrefix = "unexpected Bsd daemon error: ";
...
/**
 * Entry point shared between the new CLI `bsd daemon` and old CLI
 * `bsd-daemon`.
 *
...
static void runDaemon(bool stdio, std::optional<TrustedFlag> forceTrustClientOpt, bool processOps)
{
    if (stdio) {
        ...
        if (!processOps && (remoteStore = store.dynamic_pointer_cast<RemoteStore>()))
            ...
            // access to those is explicitly not `bsd-daemon`'s responsibility.
            processStdioConnection(store, forceTrustClientOpt.value_or(Trusted));
    } else
        daemonLoop(forceTrustClientOpt);
}

static int main_bsd_daemon(int argc, char * * argv)
{
    {
        auto stdio = false;
        ...
        parseCmdLine(argc, argv, [&](Strings::iterator & arg, const Strings::iterator & end) {
            if (*arg == "--daemon")
                ...
                showManPage("bsd-daemon");
            else if (*arg == "--version")
                printVersion("bsd-daemon");
            else if (*arg == "--stdio")
                stdio = true;
...
static RegisterLegacyCommand r_bsd_daemon("bsd-daemon", main_bsd_daemon);
...
struct CmdDaemon : Command
{
    ...
    CmdDaemon()
    {
        ...
        addFlag({
            ...
            .description = "Use Bsd's default trust.",
            .handler = {[&]() {
                isTrustedOpt = std::nullopt;
            }},
            .experimentalFeature = Xp::DaemonTrustOverride,
