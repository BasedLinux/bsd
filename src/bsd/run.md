...
* Run a non-default app from the `blender-bin` flake:
  ...
  ```console
  # bsd run blender-bin#blender_2_83
  ```

  Tip: you can find apps provided by this flake by running `bsd flake
  show blender-bin`.

* Run `vim` from the `bsdpkgs` flake:
  ...
  app but a package. Thus, Bsd runs the eponymous file from the `vim`
  package.

* Run `vim` with arguments:

  ```console
  # bsd run bsdpkgs#vim -- --help
  ```

# Description

`bsd run` builds and runs [*installable*](./bsd.md#installables), which must evaluate to an
*app* or a regular Bsd derivation.
...
For instance, if `name` is set to `hello-1.10`, `bsd run` will run
`$out/bin/hello`.

# Flake output attributes

If no flake output attribute is given, `bsd run` tries the following
flake output attributes:
...
If an attribute *name* is given, `bsd run` tries the following flake
output attributes:
