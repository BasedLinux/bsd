        ...
        bool executable)
{
    ...
    /* Figure out a name in the Bsd store. */
    if (!name) {
        name = baseNameOf(url);
        if (name->empty())
...
static int main_bsd_prefetch_url(int argc, char * * argv)
{
    {
        ...
        MyArgs myArgs(std::string(baseNameOf(argv[0])), [&](Strings::iterator & arg, const Strings::iterator & end) {
            if (*arg == "--help")
                showManPage("bsd-prefetch-url");
            else if (*arg == "--version")
                printVersion("bsd-prefetch-url");
            else if (*arg == "--type") {
        ...
        /* If -A is given, get the URL from the specified Bsd
           expression. */
        std::string url;
        if (!fromExpr) {
            if (args.empty())
...
static RegisterLegacyCommand r_bsd_prefetch_url("bsd-prefetch-url", main_bsd_prefetch_url);
...
struct CmdStorePrefetchFile : StoreCommand, MixJSON
{
    ...
    {
        return "download a file into the Bsd store";
    }
