    ...
    std::optional<StringMap> env)
{
    ...
    /* If this is a diverted store (i.e. its "logical" location
       ...
       (which "bsd" is), so we exec() a single-threaded helper program
    ...
    if (store->storeDir != store2->getRealStoreDir()) {
        ...
        execve(getSelfExe().value_or("bsd").c_str(), stringsToCharPtrs(helperArgs).data(), envp);
...
#else
    throw Error("mounting the Bsd store on '%s' is not supported on this platform", storeDir);
#endif
}
