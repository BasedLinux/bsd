    ...
    if (type == "app") {
        ...
        for (auto & c : context) {
            context2.emplace_back(std::visit(overloaded {
                [&](const BsdStringContextElem::DrvDeep & d) -> DerivedPath {
                ...
                [&](const BsdStringContextElem::Built & b) -> DerivedPath {
                    return DerivedPath::Built {
                        .drvPath = b.drvPath,
                        .outputs = OutputsSpec::Names { b.output },
                    };
                },
                [&](const BsdStringContextElem::Opaque & o) -> DerivedPath {
...
App UnresolvedApp::resolve(ref<Store> evalStore, ref<Store> store)
{
    ...
    if (!store->isInStore(res.program))
        throw Error("app program '%s' is not in the Bsd store", res.program);
