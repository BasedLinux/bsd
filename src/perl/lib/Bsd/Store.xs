...
MODULE = Bsd::Store PACKAGE = Bsd::Store
PROTOTYPES: ENABLE
...
StoreWrapper::queryPathHash(char * path)
    PPCODE:
        try {
            auto s = THIS->store->queryPathInfo(THIS->store->parseStorePath(path))->narHash.to_string(HashFormat::Bsd32, true);
    ...
    PPCODE:
        try {
            ...
            if (!info->deriver)
                ...
                XPUSHs(sv_2mortal(newSVpv(THIS->store->printStorePath(*info->deriver).c_str(), 0)));
            auto s = info->narHash.to_string(base32 ? HashFormat::Bsd32 : HashFormat::Base16, true);
            XPUSHs(sv_2mortal(newSVpv(s.c_str(), 0)));
    ...
    PPCODE:
        try {
            Hash h = hashPath(
                ...
                FileIngestionMethod::BsdArchive, parseHashAlgo(algo)).first;
            auto s = h.to_string(base32 ? HashFormat::Bsd32 : HashFormat::Base16, false);
            XPUSHs(sv_2mortal(newSVpv(s.c_str(), 0)));
        } catch (Error & e) {
            croak("%s", e.what());
        }
...
SV * convertHash(char * algo, char * s, int toBase32)
    PPCODE:
        try {
            ...
            auto s = h.to_string(toBase32 ? HashFormat::Bsd32 : HashFormat::Base16, false);
            XPUSHs(sv_2mortal(newSVpv(s.c_str(), 0)));
        } catch (Error & e) {
            croak("%s", e.what());
        }
...
StoreWrapper::addToStore(char * srcPath, int recursive, char * algo)
    PPCODE:
        try {
            auto method = recursive ? ContentAddressMethod::Raw::BsdArchive : ContentAddressMethod::Raw::Flat;
...
StoreWrapper::makeFixedOutputPath(int recursive, char * algo, char * hash, char * name)
    PPCODE:
        try {
            ...
            auto method = recursive ? FileIngestionMethod::BsdArchive : FileIngestionMethod::Flat;
...
SV * getStoreDir()
    PPCODE:
        XPUSHs(sv_2mortal(newSVpv(settings.bsdStore.c_str(), 0)));
