...
sub readManifest_ {
    ...
    if ($manifest =~ /\.bz2$/) {
        open MANIFEST, "$Bsd::Config::bzip2 -d < $manifest |"
...
sub writeManifest {
    ...
    unless (defined $noCompress) {
        system("$Bsd::Config::bzip2 < $manifest > $manifest.bz2.tmp") == 0
...
sub fingerprintPath {
    ...
    die if substr($storePath, 0, length($Bsd::Config::storeDir)) ne $Bsd::Config::storeDir;
    ...
    foreach my $ref (@{$references}) {
        die if substr($ref, 0, length($Bsd::Config::storeDir)) ne $Bsd::Config::storeDir;
    }
...
sub parseNARInfo {
    ...
    if ($requireValidSig) {
        ...
        my $publicKey = $Bsd::Config::binaryCachePublicKeys{$keyName};
        ...
        eval {
            $fingerprint = fingerprintPath(
                ...
                [ map { "$Bsd::Config::storeDir/$_" } @refs ]);
        };
        if ($@) {
