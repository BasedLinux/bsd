# These overrides are applied to the dependencies of the Bsd components.

{
  # Flake inputs; used for sources
  inputs,

  # The raw Bsdpkgs, not affected by this scope
  pkgs,
...
scope: {
  ...
  # TODO Hack until https://github.com/BasedLinux/bsdpkgs/issues/45462 is fixed.
  boost =
    (pkgs.boost.override {
      extraB2Args = [
        "--with-container"
  ...
  libgit2 =
    if lib.versionAtLeast pkgs.libgit2.version "1.9.0" then
      ...
      pkgs.libgit2.overrideAttrs (attrs: {
        # libgit2: Bsdpkgs 24.11 has < 1.9.0, which needs our patches
        nativeBuildInputs =
